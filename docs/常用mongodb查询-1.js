...
db.getCollection('users').find({remote_addr:'180.171.31.192'})
...
db.getCollection('users').find({"$where": function(){
    ...
    // return reg.test(this.request)
     return this.request.indexOf("umuGlobalMod_main") > -1;
}}).count()
...
db.getCollection('users').find({"$where": function(){
    ...
    // return reg.test(this.request)
     return (this.request.indexOf("ajax/resource/getresourcepat") > -1)
}}).count()
...
db.getCollection('users').find({"$where": function(){
    ...
    // return reg.test(this.request)
     return (this.request.indexOf("script/released") > -1)
}}).count()
...
db.getCollection('users').group({
    ...
    $reduce: function(cur, prev) {
        ...
        if(cur.browser != "IE 8.0" 
            ...
            && cur.request.indexOf("error[0]=canceled") == -1  
            && cur.request.indexOf("status[0]=5") == -1  
            && cur.request.indexOf("MyAppGetHTMLElementsAtPoint") == -1  
            && cur.request.indexOf("https://drm.media.baidubce.com/v1/playerAuth") == -1 
           && cur.request.indexOf("&error[0]=canceled&") == -1   
...
db.getCollection('users').group({
    ...
    $reduce: function(cur, prev) {
        ...
        if(cur.request.indexOf("commonService.pack_f97dc64.js") > -1 ) {
           prev.count++
        }
